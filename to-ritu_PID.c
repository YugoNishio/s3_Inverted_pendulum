...
#define ZERO_ADV        500         //棒の角度が0になる時のAD値
#define ADV_TO_RAD      0.0046      //ポテンショメータの値1当たりに傾く角度[rad]
...
//***************　台車の制御　ここから　***************//
int theta_adv = 0;                                          //theta_adv=振子についているポテンショメータのAD値格納用
int left, right;                                            //left＆right=ロータリーエンコーダーの値
...
void pen_control_handler(){                                 //制御の大元デザイン
    ...
    theta = (double)(theta_adv - ZERO_ADV) * ADV_TO_RAD;    //鉛直なす角から振子までの角度[rad?]
    e = THETA_REF - theta;                                  //目標角と現在の角の誤差
    ed = (e - e0) / T;
    ei += e * T;
    e0 = e;

    if(ei > 10000) ei = 10000;                              //頭打ち処理
    if(ei < -10000) ei = -10000;

    v_ref = (e * KP + ei * KI + ed * KD);                   //  Calculate PID control ＆　(Calculate state feedback control)

    x = (float)(left + right) / 2 * PULSE_TO_METER;         //  Introduce x, dx, theta, dtheta
    dx = (x - x0) / T;
    x0 = x;
    theta = e;
    dtheta = ed;
    ...
    if(duty_ratio > 1.0) duty_ratio = 1.0;                  //指令値の頭打ち処理
    if(duty_ratio < -1.0) duty_ratio = -1.0;
    
    //**** 指令値によって発光するLEDを変える ****//
        if(duty_ratio > 0.8 )   led = 8;
        else if(duty_ratio <= 0.8 && duty_ratio >= 0)   led = 4;
        else if(duty_ratio < 0 && duty_ratio >= -0.8)   led = 2;
        else if(duty_ratio < -0.8)  led = 1;
